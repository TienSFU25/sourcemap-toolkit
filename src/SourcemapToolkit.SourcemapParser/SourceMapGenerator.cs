...
namespace SourcemapToolkit.SourcemapParser
{
    ...
    {
        public SourceMapTree ComposeSourceMaps(List<SourceMapTree> sourceMaps)
        {
            var sourcePaths = sourceMaps.Select(c => c.FullPath).ToList();
            ...
            SourceMapTree sm = new SourceMapTree
            {
                ...
                SmSources = sourceMaps
            };

            return sm;
        }

        public string SerializeIntoBase64(SourceMap sourceMap, JsonSerializerSettings jsonSerializerSettings = null)
        {
            string b = SerializeMapping(sourceMap, jsonSerializerSettings);

            byte[] byt = System.Text.Encoding.UTF8.GetBytes(b);
        ...
        public string SerializeMapping(SourceMap sourceMap, JsonSerializerSettings jsonSerializerSettings = null)
        {
            ...
            // DIRTY: add the file protocol to sources
            IEnumerable<string> sources = sourceMap.Sources;
            mapToSerialize.Sources = sourceMap.Sources.Select(c => @"file:///" + c).ToList();
